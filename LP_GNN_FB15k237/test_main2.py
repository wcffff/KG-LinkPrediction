...
# python test_main2.py -d FB15k-237 --gpu 0 --batch-size 2048 --evaluate-every 25 --n-epochs 1500 --lr 0.001 --lr-decay 0.995 --lr-step-decay 2 --n-layers 1 --d-embed 200 --num-head 64 --d-k 32 --d-v 50 --d-model 200 --d-inner 1024 --start-test-at 1000 --save-epochs 1100 --dr-enc 0.4 --dr-pff 0.2 --dr-sdp 0.1 --dr-mha 0.3 --decoder twomult
...
def predict(model, graph, device, data_iter, rel2ent_pairs, split="valid", mode="tail"):
    model.eval()
    with th.no_grad():
        results = {}
        ...
        for step, batch in enumerate(train_iter):
            ...
            pred = model(graph, sub, rel, rel2ent_pairs)
            # pred = model(triple)
...
def evaluate(model, graph, device, data_iter, rel2ent_pairs, split="valid"):
    # predict for head and tail
    left_results = predict(model, graph, device, data_iter, rel2ent_pairs, split, mode="tail")
    right_results = predict(model, graph, device, data_iter, rel2ent_pairs, split, mode="head")
    results = {}
...
def main(args):
    ...
    # model.eval()
    # model.load_state_dict(th.load("comp_link2" + "_" + args.dataset, weights_only=True))
    # valid_results = evaluate(
    #     model, graph, device, data_iter, data.rel2ent_pairs, split="valid"
    # )
    # print(
    #     "Valid MRR: {:.5}\n, MR: {:.10}\n, H@10: {:.5}\n, H@3: {:.5}\n, H@1: {:.5}\n".format(
    #         valid_results["mrr"],
    ...
    for epoch in range(args.n_epochs):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            # print(triple.shape)
            logits = model(graph, sub, rel, data.rel2ent_pairs)
        ...
        val_results = evaluate(
            model, graph, device, data_iter, data.rel2ent_pairs, split="test"
        )
        t2 = time()
        ...
        if val_results["mrr"] > best_mrr:
            ...
            th.save(
                model.state_dict(), "comp_link2" + "_" + args.dataset
            )
            kill_cnt = 0
            print("saving model...")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--decoder', default='twomult', type=str,
                        help='decoder')
    # 2411
    parser.add_argument("--seed", dest="seed", default = 2411, type=int, 
