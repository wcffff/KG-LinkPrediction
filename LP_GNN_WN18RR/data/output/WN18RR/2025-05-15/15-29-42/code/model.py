...
from decoder import ConvE, InteractE, Rel_InteractE
...
class SE_GNN(nn.Module):
    def __init__(self, h_dim):
        ...
        self.predictor = Rel_InteractE(
            h_dim, 10, 20, 64, self.n_rel * 2, self.n_ent,
            filter_sz=(3, 3), input_drop=0.3, hid_drop=0.3, feat_drop=0.3, num_filt=64
        )
        # print(self.n_rel)
        # self.predictor = CNN_Decoder(
        #     h_dim, 10, 20, 64, self.n_rel, self.n_ent,
        #     filter1_sz=(1, 3), filter2_sz=(3, 3), filter3_sz=(1, 5),
        #     input_drop=0.3, hid_drop=0.3, feat_drop=0.3
        # )
        # self.predictor = InteractE(
        #     h_dim, 10, 20, 64, self.n_rel, self.n_ent,
        #     filter_sz=(3, 3), input_drop=0.3, hid_drop=0.3, feat_drop=0.3, num_filt=64
        # )
        # loss
    ...
    def forward(self, h_id, r_id, kg):
        ...
        # (bs, n_ent)
        # print("fuck",r_id)
        score = self.predictor(head, rel, ent_emb, r_id)
...
class CompLayer(nn.Module):
    def __init__(self, h_dim):
        ...
        self.w_o = utils.get_param(h_dim, h_dim)
        self.w_i = utils.get_param(h_dim, h_dim)
    ...
    def in_out_calc(self, g, comp_emb):
        ...
        with g.local_scope():
            ...
            in_edges_idx = torch.nonzero(
                g.edata["in_edges_mask"], as_tuple=False
            ).squeeze()
            out_edges_idx = torch.nonzero(
                g.edata["out_edges_mask"], as_tuple=False
            ).squeeze()

            comp_h_O = comp_emb[out_edges_idx].mm(self.w_i)
            comp_h_I = comp_emb[in_edges_idx].mm(self.w_o)

            new_comp_h = torch.zeros(comp_emb.shape[0], self.out_dim).to(
                comp_emb.device
            )
    ...
    def forward(self, kg, ent_emb, rel_emb):
        ...
        with kg.local_scope():
            ...
            # agg
            kg.edata['comp_emb'] = self.in_out_calc(kg, kg.edata['comp_emb'])
...
class NodeLayer(nn.Module):
    ...
    def forward(self, kg, ent_emb):
        ...
        with kg.local_scope():
            ...
            # neigh_ent_emb = kg.ndata['neigh']
            kg.apply_edges(lambda edges: {'comp_emb': edges.src['emb']})
            kg.edata['comp_emb'] = self.in_out_calc(kg, kg.edata['comp_emb'])
            kg.edata['comp_emb'] = kg.edata['comp_emb'] * kg.edata['norm']
            kg.update_all(fn.copy_e('comp_emb', 'm'), fn.sum('m', 'neigh'))
...
class EdgeLayer(nn.Module):
    ...
    def forward(self, kg, ent_emb, rel_emb):
        ...
        with kg.local_scope():
            ...
            # agg
            kg.edata['emb'] = self.in_out_calc(kg, kg.edata['emb'])
