...
def get_kg(src, dst, rel, device):
    ...
    kg.edata['rel_id'] = rel
    # 创建索引数组
    indices = torch.arange(kg.num_edges())

    # 根据索引的奇偶性生成掩码
    # 奇数索引为入边（True），偶数索引为出边（False）
    in_edges_mask = (indices % 2 == 1)
    # 偶数索引为出边（True），奇数索引为入边（False）
    out_edges_mask = (indices % 2 == 0)

    # 将掩码添加到图的边数据中
    kg.edata["in_edges_mask"] = torch.Tensor(in_edges_mask)
    kg.edata["out_edges_mask"] = torch.Tensor(out_edges_mask)
    # print(kg.edata["in_edges_mask"])
